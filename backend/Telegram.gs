 ...
 * with friends ("split equal Luty", "split Luty 20, Diego 15", "split
 * none"), or rerouting it entirely — "repayment Luty" / "loan Luty" —
...
function formatEntryForTelegram_(entry, categoryName) {
  ...
  lines.push('Reply to edit — category, amount, description, paid by, currency, date, or split. ' +
    'E.g. "amount 45.50", "split equal Luty", "split Luty 20, Diego 15", "split none". ' +
    'Combine several with commas: "category groceries, amount 48, description Uber".');
  if (entry.type === 'expense') {
    lines.push('Or instead of confirming it as an expense: "repayment Luty" (you paying down what you owed them) ' +
      'or "loan Luty" (you lending them this) — either replaces it with the right Loans entry and removes it from here.');
  }
  return lines.join('\n');
}
...
// "repayment Luty" / "mark as repayment Luty" or "loan Luty" / "convert
// to loan Luty" — the Telegram equivalent of the review queue's own
...
// ("Luty 20, Diego 15") stay intact as one segment: "Diego" isn't a field
...
function applyEditCommand_(entryId, text) {
  ...
  var helpText = 'Try: "category groceries", "amount 45.50", "description text", ' +
    '"paid by Luty", "currency USD", "date 2026-09-12", "split equal Luty", ' +
    '"split Luty 20, Diego 15", or "split none" — ' +
...
// - "equal Luty" or "equal Luty, Diego" — divides the entry's CURRENT
...
// - "Luty 20, Diego 15" — explicit amount per friend, same shape
...
function parseSplitCommand_(entry, text) {
  ...
  // Custom: "Luty 20, Diego 15" — each part is "<name> <amount>".
