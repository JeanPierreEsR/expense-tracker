...
var TABLE_DEFINITIONS = {
  ...
  'Account Opening Balances': ['id', 'payment_method_id', 'currency', 'amount', 'date', 'as_of'],
...
// Same idea again, for Budgets picking up an optional `name` column —
// blank means "use the category name," same as before this existed.
function addNameColumnToBudgets() {
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Budgets');
  var headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
  if (headers.indexOf('name') !== -1) return { done: true, alreadyExisted: true };
